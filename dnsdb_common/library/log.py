...
logging_cli_opts = [
    ...
    cfg.StrOpt('log-dir',
               ...
               help='(Optional) The base directory used for relative '
                    '--log-file paths'),
    cfg.BoolOpt('timed-rotating',
                default=False,
                deprecated_name='timedrotating',
                help='(Optional)  If the log files are seperated by datetime'),
...
def _setup_logging_from_conf():
    ...
    if logpath and CONF.timed_rotating:
        baseFilename = "%s.%s.log" % (logpath, time.strftime("%Y%m%d"))
        log_timedrotating = logging.handlers.TimedRotatingFileHandler(
            baseFilename,
            when='midnight', interval=1,
            backupCount=0, encoding=None)

        log_root.addHandler(log_timedrotating)
